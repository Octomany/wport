...
parser.add_argument('-sn', help='Service name')

# Get the argument
args, unknown = parser.parse_known_args()

if not (args.p or args.sn):
    parser.parse_args(['-h'])
...
ErrorNotFound = Fore.RED + "ERROR: port not found." + Fore.WHITE

# Error message if service isn't found
ErrorServNotFound = Fore.RED + "ERROR: Service name not found." + Fore.WHITE
...
def PortNB(PN):
    ...
    if SelectedPort != ErrorNotFound:
        print("\n" + Fore.YELLOW + SelectedPort['name'] + Fore.WHITE)
        print("Port " + SelectedPort['nb']+"\n")
        print(SelectedPort['desc']+"\n")
        print(Fore.GREEN + "Pentesting tips : " + SelectedPort['link'])
        exit()
    else:
        print(ErrorNotFound)
        exit()
...
def ServiceN(SN):
    ...
    Results = next((item for item in Allports if re.search(r'(?i)\b'+SN+r'\b', item['name'])), ErrorServNotFound)
    if Results != ErrorServNotFound:
        print("\n" + Fore.YELLOW + Results['name'] + Fore.WHITE)
        print("Default port: " + Results['nb']+"\n")
        print("Description: " + Results['desc']+"\n")
        print(Fore.GREEN + "Pentesting tips : " + Results['link'])
        exit()
    else:
        print(ErrorServNotFound)
        exit()
